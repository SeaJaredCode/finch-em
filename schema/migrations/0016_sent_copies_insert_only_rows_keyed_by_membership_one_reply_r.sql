...
-- Generated: 2026-09-17T16:27:26.081Z
-- Statement count: 39
--
...
CREATE TABLE IF NOT EXISTS sent_copies (
  ...
  CONSTRAINT sent_copies_pkey PRIMARY KEY (id),
  CONSTRAINT sent_copies_membership_id_fkey FOREIGN KEY (membership_id) REFERENCES class_memberships(id) ON DELETE CASCADE,
  CONSTRAINT sent_copies_source_program_id_fkey FOREIGN KEY (source_program_id) REFERENCES programs(id) ON DELETE SET NULL
);

CREATE TABLE IF NOT EXISTS sent_copy_replies (
  sent_copy_id uuid NOT NULL,
  comment text NOT NULL DEFAULT '',
  replied_by uuid,
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT sent_copy_replies_pkey PRIMARY KEY (sent_copy_id),
  CONSTRAINT sent_copy_replies_sent_copy_id_fkey FOREIGN KEY (sent_copy_id) REFERENCES sent_copies(id) ON DELETE CASCADE,
  CONSTRAINT sent_copy_replies_replied_by_fkey FOREIGN KEY (replied_by) REFERENCES users(id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS sent_copies_membership_idx ON sent_copies(membership_id, created_at DESC);
...
GRANT DELETE, INSERT, SELECT, UPDATE ON sent_copies TO cw_runtime;

GRANT DELETE, INSERT, SELECT, UPDATE ON sent_copy_replies TO cw_runtime;
...
GRANT SELECT ON sent_copies TO cw_query;

GRANT SELECT ON sent_copy_replies TO cw_query;
...
DROP POLICY IF EXISTS cw_runtime_full_access ON sent_copies;

CREATE POLICY cw_runtime_full_access ON sent_copies AS PERMISSIVE FOR ALL TO cw_runtime USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS cw_runtime_full_access ON sent_copy_replies;

CREATE POLICY cw_runtime_full_access ON sent_copy_replies AS PERMISSIVE FOR ALL TO cw_runtime USING (true) WITH CHECK (true);
